...
def global_sp_platform_jp():
    # 创建日本平台服务商
    cus_id = new_customer('3', 'JP')
    # 查询dm_inspos库中数据同步
    ...
    # 查询commons-jp库中数据同步
    commons_jp_customer = select_action(
        testing_connect_db('commons-jp'),
        'select * from common_customer where name="服务商平台类型测试"'
    )
    commons_jp_user = select_action(
        testing_connect_db('commons-jp'),
        'select * from common_user where user_name="for_test_user"'
    )
    # 查询sp_global_test库中数据同步
    jp_customer = select_action(
        testing_connect_db('sp_global_test'),
        'select * from spg_customer where name="服务商平台类型测试"'
    )
    jp_user = select_action(
        testing_connect_db('sp_global_test'),
    ...
    b_assert.contained_text_assert(
        ...
        state='sp_global_test库spg_customer表中包含服务商“服务商平台类型测试”'
    )
    b_assert.contained_text_assert(
        jp_user,
        ['for_test_user'],
        end='@结束@',
        state='sp_global_test库spg_user表中包含用户“for_test_user”'
    )
    # 删除服务商
